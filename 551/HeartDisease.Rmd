...
Logistic regression can also be approached by bayesian modeling. In general, bayesian analysis is more flexible, and it is proved to be superior for small samples. We can incorporate prior information in bayesian modeling. For example, if we only want the most effective factors in the model, we can use some shrinkage priors to implement variable selection. 

In practice, predicting a binary response can be an application of the standard logistic regression as well as bayesian approach. In this report, we delve into a data set about heart health condition of 303 patients collected between May 1981 and September 1984 at the Cleveland Clinic in Cleveland, Ohio. The objective is to predict if the patients have heart disease based on 13 independent variables, such as age, sex, chest pain type, etc. In this process, we will explore the different effects of specific predictors to response variable in different models. In addition, we will also compare different logistic regression models.

## 2. Data Exploration

The dataset `heart.csv` is from UCI Machine Learning Repository. It has 303 observations and 14 variables in total. Every row is associated with a patient. The response variable is `target`, i.e. whether the angiographic result is present or absent of a diameter narrowing larger than 50% (presence=1, absence=0). There are 5 continuous independent variables and 8 categorical ones. See more information about the response and predictors in the data description table below.
...
Because the data is tested in a noninvasive way and is not incorporated with knowledge of other test data and historical results, there is no work-up bias, i.e., verification bias, during the data collection process.

Firstly, check the numerical summaries of the continuous and categorical variables in two separate tables below. We see that our dataset is very clean and there is no missing value to deal with. 

Table 2 shows that the continuous variables all have some extreme values, especially `chol` and `oldpeak`. The average age of the recorded patients is 54. Table 3 indicates that we have around 1/3 data from females and the rest 2/3 from males.
...
Next, take a look at the heart disease dataset through visualization. Figure 1 shows the pairwise scatterplots and histograms of continuous variables. These variables `age`, `trestbps`, `chol`, `thelach`, `oldpeak` display weak correlations between each other.

```{r,out.height='70%',out.width='70%',fig.align='center',fig.cap='Pairwise scatterplots and histogram of continuous variables.',fig.pos='H'}
...
Explore the relationship between target and the continuous variables by boxplots in Figure 2. It is noticed that the patients who are detected to have heart disease have an overall higher (`thalach`) maximum heart rate achieved. 

```{r,out.height='55%',out.width='70%',fig.align='center',fig.cap='Boxplots of continuous variables.',fig.pos='H'}
...
Take a quick look at the bar charts of categorical variables against response variable in Figure 3. As we could see, the response `target` is relatively balanced.
...
Before we perform logistic regressions to dataset, we need to prepare our data first. 

First, we turn categorical variables into dummy variables by using function `model.matrix`. The first column of the result matrix is intercept 1. As a result, the number of independent variables grow from 13 to 23.

In order to test the prediction performance of the models, we then randomly split the whole dataset into training (80%) and test (20%) parts. Our training dataset has 273 observations and test dataset has 30 observations.
```{r}
...
where $x_i$ is the i-th row of observations and $\beta,\beta_0$ are parameters.

Using `glm` function, we could easily obtain estimates of $\beta$ parameters. Table 4 provides us some summary statistics of the fitting result. From p.value column, we could observe that only 6 out of 23 predictors are significant in our model. If the significance level is $\alpha=0.1$, there are still only 9 significant predictors, which encourages us to seek sparse solutions.
...
In the standard logistic regression above, we treat $\beta$ as a column of unknown but fixed parameters. Actually, $\beta,\beta_0$ could be seen as a vector of random variables from the prospective of bayesian analysis. In this way, we could combine data (model) and the prior we build up to obtain not only a point estimate, but also the posterior distribution of parameters. In addition to basic assumptions of standard logistic regression, bayesian logistic regression method with N-IG prior assumes that $\beta,\beta_0$ subject to a normal prior with the variance following an Inverse-Gamma distribution. Other parameters are assumed to be flat because we don't have much information about them.
...
Using a powerful tool `Rstan` in R, we implement Monte Carlo Markov Chain algorithm to our model and obtain posterior samples of parameters $\beta$. We naturally compute their 95% credible intervals based on the sample quantiles. The plot below shows the comparision of the corresponding parameters obtained by standard logistic regression and bayesian logistic. The red lines represent the $\beta$ estimated from standard logistic regression and green lines represent the 95% credible intervals produced by posterior samples of $\beta$, while blue lines indicate the location of 0.
...
Most red lines from standard logistic regression fall into the 95% credible intervals of posterior samples, meaning that most variables produce similar effects in both models. In contrast, the absolute values of the coefficients of variables `intercept`, `thal1`, `thal2` and `thal3` in bayesian logistic regression are much smaller than the ones in standard model. This may result from the fact that bayesian logistic regression could make full use of the information in data and adjust the effects of these variables properly.

### 3.3 Bayesian Logistic Regression with NEG prior

Recall that in Table 4 most variables actually do not produce significant effects to the response. In order to figure out the most important predictors to the `target` response and to improve the interpretation of our results, we choose to use bayesian logistic models with shrinkage priors. There are many priors which are proved to have shrinkage effects to the parameters, for example, Cauchy prior, Laplace prior and horseshoe prior. We choose Normal-Exponential-Gamma prior considering the consistency with two previous models.
...
Figure 7 shows point estimate and estimated confidence intervals of parameters $\beta,\beta_0$ from all three different models above.
...
From this plot, we could observe that NEG prior does have strong shrinkage effects on the coefficients that 0 is included in most of blue confidence intervals. We also notice that the variables kept are exactly the same significant variables in standard logistic regression. 
...
Based on standard logistic regression model, it is easy to make predictions using function `predict`. We make predictions and obtain the confusion matrix with 22 right predictions and 8 wrong ones as below.
...
If we want to use the fitted bayesian logistic regression models to make predictions for comparison, we should sample from the posterior distributions of $\beta,\beta_0$, as the posterior distribution has beed updated by incorporating information from data and prior. To do this, we first randomly draw samples of $\beta$ from the posterior distribution many times. And then we generate new $y$ which has the same size of responses as the test dataset for each sample of $\beta$. In this way, we could obtain many groups of predictions for test data. Finally, we compare each group of predicted responses with the original test data.

In the next step, we are going to use two indices `log-loss` and `sensitivity` to evaluate the prediction performances of the three models.
...
Log-loss or logarithmic loss is a metric used in classification problems to compare the prediction performances of different models. The lower the log-loss is, the better the performance is. Log-loss also requires the raw probabilities $P(y=1|\beta,\beta_0)$, as well as the true response. For logistic regression, log-loss is defined as:
...
```{r,out.height='50%',out.width='60%',fig.align='center',fig.cap='Histogram of predictive log-loss',fig.pos='H'}
...
```{r,out.height='50%',out.width='60%',fig.align='center',fig.cap='Histogram of predictive sensitivity',fig.pos='H'}
...
In conclusion, bayesian logistic regression model with shrinkage prior (NEG prior) outperforms other two models in prediction. The bayesian logistic regression models both have a better prediction performance than the standard logistic regression, which indicates the significance and powerfulness of the bayesian analysis. 
...
Genkin, A., Lewis, D. & Madigan, D. (2007). Large-scale Bayesian logistic regression for text categorization. Technometrics, 49(3): 291?C304.
