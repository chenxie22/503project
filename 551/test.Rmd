...
In the standard logistic regression above, we treat $\beta$ as a column of unknown but fixed parameters. Actually, $\beta$ could be seen as a vector of random variables from the prospective of Bayesian analysis. That is the preliminary thought of Bayesian analysis. In this case, we can also get a point estimate for the parameters by maximizing the posterior distribution, that is, maximizing $P(\beta|y_1,...,y_n)\propto P(y_1,...,y_n;\beta)P(\beta)$. The maximum of posterior distribution, that is, the mode of posterior, is called MAP estimation. We can see theat the ML estimate of standard logistic regression is the MAP estimate when the prior of $\beta$ is uniform. 
...
By normal approaximation, we can get Estimate, standard errors, z statistics, and p-values of every $\beta$, as Table 4 shows. We can find ...

```{r}
data.frame(coef(summary(res_original))) %>%
  kable(col.names = c('Estimate','Std.Error','z.value','p.value'),
        caption = 'Summary of Logistic Regression Fit',align = 'l',format="latex",longtable=TRUE,booktabs = T) %>%
  kableExtra::kable_styling(latex_options = "striped") 
```
...
```{r}
data.frame(v1=c('Actual: No','Actual: Yes'),v2=c(1942, 389),v3=c(71, 98)) %>%
  kable(align = 'r',col.names = c('','Predicted: No','Predicted: Yes'), 
        caption = 'Prediction Result of Logistic Regression')
```
...
```{r}
data.frame(v1=names(res_original$coefficients),
           v2=unname(beta2_mean),
           v3=unname(beta2_sd),
           v4=unname(beta2_mean/beta2_sd),
           v5=unname(p2)) %>%
  kable(col.names = c('','Estimate','Std.Error','z.value','p.value'),
        caption = 'Summary of Bayesian Logistic with Normal Prior',
        align = 'l',format="latex",longtable=TRUE,booktabs = T) %>%
  kableExtra::kable_styling(latex_options = "striped") 
```
...
data.frame(v1=c('Actual: No','Actual: Yes'),v2=c(1942, 389),v3=c(71, 98)) %>%
  kable(align = 'r',col.names = c('','Predicted: No','Predicted: Yes'), 
        caption = 'Prediction Result of Bayesian Logistic with Normal Prior')
```
...
```{r}
data.frame(v1=c('Actual: No','Actual: Yes'),v2=c(1947, 392),v3=c(66, 95)) %>%
  kable(align = 'r',col.names = c('','Predicted: No','Predicted: Yes'), 
        caption = 'Prediction Result of Bayesian Logistic with Normal Prior')
```
...
knitr::kable(p,col.names = c('p1','p2'),
        caption = 'Comparison',align = 'l',format="latex",longtable=TRUE,booktabs = T) %>%
  kableExtra::kable_styling(latex_options = "striped") 
```
