...
In order to verify our guess above that the decision boundary is non-linear, we first try two linear models, i.e., linear multinomial logistic regression and LDA. Both multinomial logistic regression and LDA are powerful models that we usually use in machine learning when the decision boundary is approximately linear. Multinomial logistic regression is a generalization of original logistic regression when the response has multiple classes. And LDA assumes data of each class follows a normal distribution with identical variance.

Based on the fitted models, the prediction accuracy of multinomial logistic regression and LDA on test data set are only 34.5% and 34.8% respectively, which is quite disappointing. But we could not simply conclude that our decison boundary to be non-linear this time, we are encouraged to try non-linear version models of multinomial logistic regression and LDA in the next step. If these models show great improvement regarding prediction performance, we could confirm our initial guess and try other flexible models.


### 3.1 Quadratic discriminant analysis (QDA)

QDA is a more flexible discriminant method based on LDA. Comapred to LDA, QDA relaxes the limitation of the same variance of normal distribution. It could thus be applied to more scenarios and larger range of datasets. In this step, we first fit QDA classifiers on this dataset and then discuss about our prediction result together with multinomial logistic regression.
...
From the confusion matrix, we could see that QDA has a very good prediction performance with its prediction accuracy over 90.9%. The classifier QDA with quadratic classification boundary has an overwhelming advantage over the classifier LDA with linear boundary regarding the prediction accuracy. In addition, we see from Table 3 that adding quadratic terms in the logistic regression model greatly increase the prediction accuracy, from previous 34.5% to current 88.8%. As quadratic models improve the prediction accuracy greatly, our expectation in data exploration is confirmed that the decision boundary between classes is not linear.

### 3.3 K-Nearest Neighbors (KNN)

As we are sure that the decision boundary is non-linear, we want to try more flexible models with less assumptions on the distribution of the dataset. Therefore, we try KNN method on this dataset. First, we perform 5-fold cross validation on training set to choose the corresponding K with the smallest cross-validation error. From Figure 6, we could see that K equals 3 is the optimal choice. After that, we fit KNN model with K=3 again on the test set and obtain the prediction accuracy.
...
```
The accuracy of 84.63% indicates that KNN (K=3) also performs well on this dataset, although its performance is a little worse than the logistic regression with quadratic terms and QDA. The reason is that KNN method has difficulties in dealing with high dimensional data as mentioned before in data exploration part. 

### 3.4 Support Vector Machine (SVM)

Fit the SVM classifiers with radial kernel and polynomial kernel to the dataset. The optimal parameters such as cost, gamma, degree are chosen by minimizing the validation errors. Note that here we use validation instead of cross validation under consideration of the computational cost. For the SVM with radial kernel, the optimal parameters chosen are that cost = 1, gamma = 0.5 . For the SVM with polynomial kernel, the optimal parameters cost = 100, degree = 2. 
...
Table 6 shows that SVM with these two kernels both perform well, with the one with polynomial kernel having higher accuracy rate than the one with the radial kernel. We see that the SVM with polynomial kernel has optimal degree 2 and relatively better prediction performance, indicating that the decision boundary between classes probably does have quadratic curves. 

### 3.5 Classification Tree (CART) & Random Forest (RF)

Finally we fit classification tree and random forest to the dataset. 
...
The ultimate decision tree model is shown in Figure 7. We see that it uses the variables 'V63', 'V58', 'V31', 'V15', 'V50' and 'V47'. The decision tree has accuracy slightly larger than 70%, which is hardly counted as good performance. This intrigues us to do random forest to better the prediction performance. 
...
The random forest has the best prediction accuracy so far which is over 91.1%. Becuase partial plots of 8 readings from each sensor display a similar pattern, we just show the partial plots of `V7` and `V18` in Figure 8 as the representatives of 7th and 2nd sensor. Keeping other predictors unchangeable, the influence of the 7th and 2nd sensors to the gesture classes varies from time to time. It is not linear nor constant, so these senors produce a significant and non-linear influence to the response. In general, the effects of sensors on response exhibit different patterns when they are closed to 0 and away from 0. 
...
Table 8 collects the prediction accuracy rates of all the classification methods we go through. We can see that random forest has the best prediction performance, QDA, SVM with degree-2 polynomial kernel and logistic regression with quadratic terms of the predictors have the next best performance.
...
In order to reduce dimensionality, we use the random forest and stepwise forward selection method to do variable selection. We find that the motion of the muscles corresponding to the 2nd and 7th sensors are almost decisive for these 4 gestures. 

Among all the classification methods we try, random forest performs the best, the classifiers with quadratic decision boundary such as QDA, SVM with degree-2 polynomial kernel and logistic regression with quadratic terms all perform quite well. 
