...
```{r}
set.seed(3)
lr = multinom(V65~., data=g_new[train,])
pred_lr = predict(lr, g_new[-train,])
rate_lr=mean(pred_lr == g_new[-train,'V65'])
rate_lr
```
...
```{r}
library(MASS)
lda.fit = lda(V65~., data=g_new[train,])
lda.pred = predict(lda.fit, g_new[-train,])
rate_lda=mean(lda.pred$class == g_new[-train,'V65'])
rate_lda
```
...
knn_train = NULL
knn_test = NULL
k_list = c(1:9,seq(10,100,10))
set.seed(3)
for(k in k_list) {
    knn_tr = knn(g_new[train,-17], g_new[train,-17], g_new[train,17], k = k)
    knn_t = knn(g_new[train,-17], g_new[-train,-17], g_new[train,17], k = k)
    
    knn_train = c(knn_train, mean(knn_tr == g_new[train,'V65']))
    knn_test = c(knn_test, mean(knn_t == g_new[-train,'V65']))
}
```

```{r, fig.cap=cap, fig.align = "center"}
cap = 'The training and test errors for each choice of K.'
plot(k_list, knn_train, col='blue', type='l', ylim=c(0.68,1),
     xlab='Number of Nearest Neighbors K', ylab='Error')
lines(k_list, knn_test, col='red')
points(k_list[which.max(knn_test)], max(knn_test))
legend(70, 1, c('training error','test error'), col=c('blue','red'), lty=c(1,1))
```
...
```{r}
set.seed(77)
rf_2=randomForest(V65~.,data=g_new[train,],mtry=4,importance=TRUE)
rf_pred_2 = predict(rf_2,newdata=g_new[-train,])
table(rf_pred_2,g_new$V65[-train])
rate_tree_2=mean(rf_pred_2==g$V65[-train])
rate_tree_2
```
